...
public class Consultas{
	...
	public boolean registrarUsuario(DatosDeUsuario ddu){
		...
		try {
			...
			if(existe==false){
				...
				psInsertar=(PreparedStatement) con.prepareStatement("INSERT INTO Usuario (Nombre,Email,Contrase�a,Genero,Altura_cm,Peso_kg,Objetivo,Actividad,Edad) VALUES (?,?,?,?,?,?,?,?,?)");
	...
	public boolean actualizarUsuario(DatosDeUsuario ddu){
		...
		try {		
				psInsertar=(PreparedStatement) con.prepareStatement("UPDATE Usuario SET Email=?,Contrase�a=?,Genero=?,Altura_cm=?,Peso_kg=?,Objetivo=?,Actividad=?,Edad=? WHERE Nombre LIKE ?");
		...
		try {
			...
			if(existe==true){
				...
				try {
					...
					cmd = (Statement) con.createStatement();
					rs = cmd.executeQuery("SELECT contrase�a FROM Usuario WHERE Nombre LIKE "+"'"+nom+"'");
					
					while (rs.next()) {
						if(pass.equals(rs.getString("Contrase�a"))){
							correcta=true;
						}					
	...
	public String[] consultarStringUsuario(String nombre){	
		...
		try {
			...
			Statement cmd = null;
			cmd = (Statement) con.createStatement();
			rs = cmd.executeQuery("SELECT Nombre,Email,Contrase�a FROM Usuario WHERE Nombre LIKE "+"'"+nombre+"'");
			
			while (rs.next()) {
				stringUsuario[0]=rs.getString("Nombre");
				stringUsuario[1]=rs.getString("Email");
				stringUsuario[2]=rs.getString("Contrase�a");
